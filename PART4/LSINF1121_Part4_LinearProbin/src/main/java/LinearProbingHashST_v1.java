public class LinearProbingHashST_v1<Key, Value> {
    ...
    public LinearProbingHashST_v1() {
        this(INIT_CAPACITY);
    }

    public LinearProbingHashST_v1(int capacity) {
        m = capacity;
        n = 0;
    ...
    private void resize(int capacity) {
        LinearProbingHashST_v1<Key, Value> nLPST = new LinearProbingHashST_v1(capacity);
        for(int i=0; i<m; i++){
            if(keys[i] != null)
                nLPST.put(keys[i], vals[i]);
        }
        this.keys = nLPST.keys;
        this.vals = nLPST.vals;
        this.m = nLPST.m;
    }
